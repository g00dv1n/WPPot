...
function mdocs_noonce_end_session() {
	if(session_id()) {
		session_destroy ();
	}
}
...
function mdocs_doc_preview($file) {
	if(get_option('mdocs-preview-type') == 'google') {
		...
		$link = site_url().'/?mdocs-file='.$file['id'];
		?>
		<iframe id="mdocs-box-view-iframe" src="//drive.google.com/viewerng/viewer?url=<?php echo $link; ?>&embedded=true" style="border: none; width: 100%;" seamless fullscreen></iframe>
		<script>
